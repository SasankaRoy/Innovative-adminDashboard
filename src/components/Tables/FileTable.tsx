...
import { createFileTemplates, deleteFileTemplates, updateFileTemplates } from '../../api-calls/apicalls';
    ...
    <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <nav className="flex justify-center items-center  gap-5 w-[40%]">
        ...
        <ol className="flex items-center gap-2">
          ...
          <li className="font-medium text-primary">File Template Management</li>
        </ol>
      </nav>
    </div>
      ...
      <div className="fixed top-0 left-0 EditModelZindex flex justify-center items-center w-full h-full backdrop-blur-md">
        <div className="shadow-md p-4 w-[50%] rounded-md dark:border-strokedark dark:bg-boxdark border-stroke bg-white overflow-y-auto max-h-full">
          ...
          <div className="grid grid-cols-1 gap-y-8">
            <div>
              <label className="block pb-1 text-sm font-medium text-gray-700">Template Name</label>
              <input type="text" className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Template Name" value={templateName}
                onChange={(e) => {
            ...
            <div>
              <label className="block pb-1 text-sm font-medium text-gray-700">Template Description</label>
              <input type="text" className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Template Description" value={templateDesc}
                onChange={(e) => { setTemplateDesc(e.target.value); }} />
            </div>
            {dbPdfs.length !== 0 && <div>
              <label className="block pb-1 text-sm font-medium text-gray-700">Already Attached Pdfs</label>
              {dbPdfs.map((dp, ind) => (
                <div className="flex items-center space-x-2">
                  <span>{dp?.file_name}</span>
                  <div >
                    <input type="checkbox" role="switch" id={`flexSwitch-db-pdf-wm-${ind}`} onChange={() => { handleSubBtns(`flexSwitch-db-pdf-wm-${ind}`); }} />
                    <label htmlFor={`flexSwitch-db-pdf-wm-${ind}`}>Watermark</label>
                  </div>
                  <div >
                    <input type="checkbox" role="switch" id={`flexSwitch-db-pdf-lb-${ind}`} onChange={() => { handleSubBtns(`flexSwitch-db-pdf-lb-${ind}`); }} />
                    <label htmlFor={`flexSwitch-db-pdf-lb-${ind}`}>Top Left Logo</label>
                  </div>
                  <div>
                    <input type="checkbox" role="switch" id={`flexSwitch-db-pdf-pn-${ind}`} onChange={() => { handleSubBtns(`flexSwitch-db-pdf-pn-${ind}`); }} />
                    <label htmlFor={`flexSwitch-db-pdf-pn-${ind}`}>Bottom Right Page No</label>
                  </div>
                  <div>
                    <input type="checkbox" role="switch" id={`flexSwitch-db-pdf-do-${ind}`} onChange={() => { handleSubBtns(`flexSwitch-db-pdf-do-${ind}`); }} />
                    <label htmlFor={`flexSwitch-db-pdf-do-${ind}`}>Download Option</label>
                  </div>
                </div>
              ))}
            </div>}
            <div>
              <div className="flex justify-between items-center">
                <label className="block pb-1 text-sm font-medium text-gray-700">Template Pdfs</label>
                <button className="btn" style={{ width: "fit-content", background: "#cfb0cc", whiteSpace: "nowrap", }} onClick={() => { handlePdfsInputCnt(); }} >
                  Add Pdfs <AddIcon />
                </button>
              </div>
              {pdfsCnt.length !== 0 && pdfsCnt.map((data, ind) => (
                <>
                  <div className="flex mt-2 space-x-2">
                    <input type="text" id={`pdf-title-${ind}`} className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Pdf Title" />
                    <input type="file" id={`add-pdfs-${ind}`} className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Template Pdfs" onChange={(e) => { pdfFilesHandler(e, ind); }} />
                  </div>
                  {templatePdfs[ind] && (
                    <div className="flex items-center space-x-2">
                      <span>{templatePdfs[ind]?.name}</span>
                      <div >
                        <input type="checkbox" role="switch" id={`flexSwitch-pdf-wm-${ind}`} onChange={() => { handleSubBtns(`flexSwitch-pdf-wm-${ind}`); }} />
                        <label htmlFor={`flexSwitch-pdf-wm-${ind}`}>Watermark</label>
                      </div>
                      <div >
                        <input type="checkbox" role="switch" id={`flexSwitch-pdf-lb-${ind}`} onChange={() => { handleSubBtns(`flexSwitch-pdf-lb-${ind}`); }} />
                        <label htmlFor={`flexSwitch-pdf-lb-${ind}`}>Top Left Logo</label>
                      </div>
                      <div >
                        <input type="checkbox" role="switch" id={`flexSwitch-pdf-pn-${ind}`} onChange={() => { handleSubBtns(`flexSwitch-pdf-pn-${ind}`); }} />
                        <label htmlFor={`flexSwitch-pdf-pn-${ind}`}>Bottom Right Page No</label>
                      </div>
                      <div >
                        <input type="checkbox" role="switch" id={`flexSwitch-pdf-do-${ind}`} onChange={() => { handleSubBtns(`flexSwitch-pdf-do-${ind}`); }} />
                        <label htmlFor={`flexSwitch-pdf-do-${ind}`}>Download Option</label>
                      </div>
                    </div>
                  )}
                </>
            ...
            {dbZips.length !== 0 && <div>
              <label className="block pb-1 text-sm font-medium text-gray-700">Already Attached Zips</label>
              {dbZips.map((dz, ind) => (
                <div className="flex items-center space-x-2">
                  <span>{dz?.file_name}</span>
                  <div >
                    <input type="checkbox" role="switch" id={`flexSwitch-db-zip-do-${ind}`} onChange={() => { handleSubBtns(`flexSwitch-db-zip-do-${ind}`); }} />
                    <label htmlFor={`flexSwitch-db-zip-do-${ind}`}>Download Option</label>
                  </div>
                </div>
              ))}
            </div>}
            <div>
              <div className="flex justify-between items-center">
                <label className="block pb-1 text-sm font-medium text-gray-700">Template Zips</label>
                <button className="btn" style={{ width: "fit-content", background: "#cfb0cc", whiteSpace: "nowrap", }} onClick={() => { handleZipsInputCnt(); }} >
                  add zips <AddIcon />
                </button>
              </div>
              {zipsCnt.length !== 0 && zipsCnt.map((data, ind) => (
                <>
                  <div className="flex mt-2 space-x-2">
                    <input type="text" id={`zip-title-${ind}`} className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Zip Title" />
                    <input type="file" id={`add-zips-${ind}`} className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Template Zips" onChange={(e) => { zipFilesHandler(e, ind); }} />
                  </div>
                  {templateZips[ind] && (
                    <div className="flex items-center space-x-2">
                      <span>{templateZips[ind]?.name}</span>
                      <div >
                        <input type="checkbox" role="switch" id={`flexSwitch-zip-do-${ind}`} onChange={() => { handleSubBtns(`flexSwitch-zip-do-${ind}`); }} />
                        <label htmlFor={`flexSwitch-zip-do-${ind}`}>Download Option</label>
                      </div>
                    </div>
                  )}
                </>
            ...
            {dbLinks.length !== 0 && <div>
              <label className="block pb-1 text-sm font-medium text-gray-700">Already Attached Links</label>
              {dbLinks.map((dl, ind) => (
                <div className="flex space-x-2">
                  <input placeholder="db link preview name" id={`db-link-name-${ind}`} className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
                  <input placeholder="db link url" id={`db-link-url-${ind}`} className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
                </div>
              ))}
            </div>}
            <div>
              <div className="flex justify-between items-center">
                <label className="block pb-1 text-sm font-medium text-gray-700">Template Links</label>
                <button className="btn" style={{ width: "fit-content", background: "#cfb0cc", whiteSpace: "nowrap", }} onClick={() => { handleLinks(); }} >
                  add links <AddIcon />
                </button>
              </div>
              {links.length !== 0 && links.map((data, ind) => (
                <div className="flex mt-2 space-x-2">
                  <input type="text" id={`link-name-${ind}`} className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Template Links Name" />
                  <input type="text" id={`link-url-${ind}`} className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Template Links Url" />
                </div>
              ))}
            </div>
          </div>
